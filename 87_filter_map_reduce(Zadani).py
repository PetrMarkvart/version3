...
long_range_aircrafts = list(filter(lambda x: , aircrafts))
print("Letadla s doletem nad 10 000 km:", long_range_aircrafts)

#Získat názvy všech letadel:
aircraft_models = list(map(lambda x: x[0], aircrafts))
print("Modely letadel:", aircraft_models)


from functools import reduce

total_capacity = reduce(lambda acc, x: ___________, aircrafts, 0)
